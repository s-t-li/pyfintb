...
将各数据源返回的数据全部统一转换为pandas DataFrame/Series数据格式，并针对单日期、单字段等数据进行自动格式优化转换。统一数据格式如下:
...
``` python
wind_series(wcode, field, start_date, end_date=TODAY, **kwargs)
```
...
#### 1.1.3 查询宏观经济（Wind EDB）时间序列数据

``` python
wind_edb(wcode, start_date, end_date=TODAY, period=None, **kwargs)
```
